...
export class CoreService extends API {
  ...
  async getSwapTask() {
    try {
      ...
      const response = await this.fetch("https://task-api.testnet.mangonetwork.io/base/taskDetail", 'POST', {
        'taskId': 2,
        'type': 0
      }, this.token);
      if (response.data.code == 0) {
  ...
  async getFaucet() {
    try {
      ...
      const response = await this.fetch("https://task-api.testnet.mangonetwork.io/base/getFaucet", 'POST', {
        'chain': '0',
        'type': false
      }, this.token);
  ...
  async swap(fromCoin, toCoin) {
    try {
      ...
      await Helper.delay(1000, this.acc, "Try to Swapping " + (fromCoin == COINS.MGO ? parseFloat((Number(amount) / Number(MIST_PER_MGO)).toString()).toFixed(2) : parseFloat((Number(coins.data[0].balance) / Number(MIST_PER_MGO)).toString()).toFixed(5)) + " " + fromCoin.SYMBOL + " to ? " + toCoin.SYMBOL, this);
      const coinTypes = [fromCoin, toCoin].find(coin => coin == COINS.MGO);
      const typeArguments = fromCoin == COINS.MGO || !coinTypes && fromCoin == COINS.USDT ? [fromCoin.TYPE, toCoin.TYPE] : [fromCoin.TYPE, toCoin.TYPE].reverse();
      const poolId = await this.getPool(typeArguments);
      let swapAmount = await this.swapCalculate(typeArguments, poolId, !!(fromCoin == COINS.MGO || !coinTypes && fromCoin == COINS.USDT), amount);
      swapAmount = Math.floor(swapAmount - swapAmount * 10 / 100);
      await Helper.delay(1000, this.acc, "Try to Swapping " + parseFloat((Number(amount) / Number(MIST_PER_MGO)).toString()).toFixed(2) + " " + fromCoin.SYMBOL + " to " + parseFloat((Number(swapAmount) / Number(MIST_PER_MGO)).toString()).toFixed(2) + " " + toCoin.SYMBOL, this);
      txBlock.moveCall({
        'target': AMMPACKAGE.ADDRESS + "::amm_script::" + (fromCoin == COINS.MGO || !coinTypes && fromCoin == COINS.USDT ? "swap_exact_coinA_for_coinB" : "swap_exact_coinB_for_coinA"),
      ...
      await this.executeTx(txBlock);
      await Helper.delay(1000, this.acc, "Successfully Swapping " + parseFloat((Number(amount) / Number(MIST_PER_MGO)).toString()).toFixed(2) + " " + fromCoin.SYMBOL + " to " + parseFloat((Number(swapAmount) / Number(MIST_PER_MGO)).toString()).toFixed(2) + " " + toCoin.SYMBOL, this);
    } catch (error) {
      throw error;
    }
  }
  ...
  async exchange(fromCoin, toCoin) {
    try {
      ...
      const typeArguments = fromCoin == COINS.USDT ? [fromCoin.TYPE, toCoin.TYPE].reverse() : [fromCoin.TYPE, toCoin.TYPE];
      const txBlock = new TransactionBlock();
      let coins = await this.client.getCoins({
        'owner': this.address,
        'coinType': fromCoin.TYPE
      ...
      txBlock.moveCall({
        'target': BEINGDEXPACKAGE.ADDRESS + "::clob::" + (fromCoin == COINS.USDT ? "market_buy" : "sell"),
        'typeArguments': typeArguments,
        'arguments': fromCoin == COINS.USDT ? [txBlock.object(BEINGDEXPACKAGE.MODULE.CLOB.AIUSDTPOOL), txBlock.object(coins.data[0].coinObjectId), txBlock.pure(amount)] : [txBlock.object(BEINGDEXPACKAGE.MODULE.CLOB.AIUSDTPOOL), txBlock.object(coins.data[0].coinObjectId), txBlock.pure(1), txBlock.pure(amount)]
      });
      await this.executeTx(txBlock);
      await Helper.delay(1000, this.acc, "Successfully Exchanging " + parseFloat((Number(amount) / Number(MIST_PER_MGO)).toString()).toFixed(2) + " " + fromCoin.SYMBOL + " to " + toCoin.SYMBOL, this);
    } catch (error) {
      throw error;
    }
  }
  ...
  async mergeCoin(coin) {
    try {
      ...
      await txBlock.mergeCoins(txBlock.object(primaryCoin), coinsToMerge.map(c => txBlock.object(c)));
      await this.executeTx(txBlock);
      await this.getBalance();
    } catch (error) {
      throw error;
  ...
  async bridge(destination) {
    try {
      if (destination == BRIDGE.MANGOBSC || destination == BRIDGE.MANGOETH) {
        ...
        await Helper.delay(1000, this.acc, "Try to Bridge " + parseFloat((Number(amount) / Number(MIST_PER_MGO)).toString()).toFixed(2) + " " + COINS.USDT.SYMBOL + " to BSC NETWORK ", this);
        txBlock.moveCall({
          'target': MANGOBRIDGEPACKAGE.ADDRESS + "::bridge::bridge_token",
          'typeArguments': [COINS.USDT.TYPE],
          'arguments': [txBlock.object(MANGOBRIDGEPACKAGE.MODULE.BRIDGE.BRIDGEXECUTOR), splitCoin, txBlock.pure('0x1f0ea6e0b3590e1ab6c12ea0a24d3d0d9bf7707d'), txBlock.pure(destination), txBlock.object(MANGOBRIDGEPACKAGE.MODULE.BRIDGE.CLOCK)]
        });
        await this.executeTx(txBlock);
        await Helper.delay(1000, this.acc, "Successfully Bridge " + parseFloat((Number(amount) / Number(MIST_PER_MGO)).toString()).toFixed(2) + " " + COINS.USDT.SYMBOL + " to BSC NETWORK", this);
      }
    } catch (error) {
      throw error;
    }
  ...
  async swapCalculate(typeArguments, poolId, isExactInput, amount) {
    ...
    txBlock.moveCall({
      ...
      'typeArguments': typeArguments,
      'arguments': [txBlock.object(poolId), txBlock.pure(amount), txBlock.pure(isExactInput)]
    });
  ...
  async executeTx(txBlock) {
    try {
      ...
      await Helper.delay(3000, this.acc, "Tx Executed : " + (this.explorer + "/txblock/" + result.digest), this);
      await this.getBalance();
      return result;
    } catch (error) {
      throw error;
